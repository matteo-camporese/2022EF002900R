...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function[CcF,An,rs,Rdark,gsCO2,J,NPQ,Fvp,Fmp] = photosynthesis(Cc,IPAR,PPFD,Csl,ra,rb,Ts,Pre,RH,...
    CT,T0,Vmax,Oa,g1,go,rjv)
...
%%% IPAR [umol/m^2 s] Absorbed photosynthetic photon flux density
%%% PPFD [umol electron/m^2 s] Used photosynthetic photon flux density
...
%%% FI Intrinsic quantum Efficiency [umolCO2/umolPhotons] -> don't need
%%% this at the moment
%%% Oa  Intercellular Partial Pressure Oxygen [umolO2/mol] -
%%% gsCO2  = [ umolCO2 / s m^2 ] Stomatal conductance to CO2
%%% gmes =  Mesophyl Conductance [ molCO2 / s m^2 ] -> infinite at the
%%% moment
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Ta=Ts;
% es=0.6108*exp(17.27*Ta/(Ta+237.3))*1000; % saturation vapor pressure [Pa]
% Ds=es*(1-RH/100); % vapor pressure deficit [Pa]
es=0.6108*exp(17.27*Ta/(Ta+237.3)); % saturation vapor pressure [kPa]
Ds=es*(1-RH/100); % vapor pressure deficit [kPa]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% OUTPUT
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
rmes = 1/(1e+6*0.60) ; %% [ s m^2 /umolCO2 ] Mesophyl Resistance
...
Ts_k = Ts + 273.15; %%[K]
Tref = T0 + 273.15; %% [K] Reference Temperature
R =   0.008314; %%  [kJ·/ K mol] Gas Constant
%%%
ANS_TEMP=1;
if  ANS_TEMP == 1 %% Kattge and Knorr 2007
    Hd =  202.9 ;% [kJ/mol]  Deactivation Energy -- constant
    Ha = 116.3; %[kJ/mol] Activation Energy - Plant Dependent
    DS = 0.650; %[kJ / mol K]  entropy factor - Plant Dependent
    ...
    %rjv= 1.97; %%[]
    Hd =  201;%  [kJ/mol]  Deactivation Energy -- constant
    Ha = 79.5;% [kJ/mol] Activation Energy
    DS = 0.650;% [kJ / mol K]  entropy factor
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    kT =exp(Ha*(Ts_k-Tref)./(Tref*R*Ts_k)).*(1+exp((Tref*DS - Hd)/(Tref*R)))./(1+exp((Ts_k*DS-Hd)./(Ts_k*R)));
...
switch ANSG
    ...
    case 1
        ...
        Tref = 293.2; %%[K]
        %%%%
        GAM = G0*(1 +G1*(Ts+273.15 -Tref) + G2*(Ts+273.15-Tref)^2); %%% [umolCO2/molAIR] -- CO2 Compensation point - Leuning 1995
...
switch ANSMOD
    case 1 % Fatichi
        Q = 0.081*IPAR; %%  [umolCO2/ s m^2 ] %% Light Absorbed by Photosystem II in CO2 units
        ...
        elseif CT==4
            ...
            %%% Light Limited
            JL = J; %%% Gross Assimilation Rate Limited by Light % [umolCO2/ s m^2 ]
            %%% PEP Carboxylase Limited
            JE = ke*Cc/Pre;
            %JE =  20000*Vm*(Cc/Pre)*(CT==4); %% Gross Assimilation Rate Limited by Transport % [umolCO2/ s m^2 ]
        end
    case 2 % Daly
        Q = PPFD; %%  [umol/ s m^2 ] %% Light Absorbed by Photosystem II in photon units
        d1= 0.74; d2= -(0.79*Q + Jm); d3= 0.79*Q*Jm ;  %% Leuning 1995
        %%% ELectron Transport Rate
        J=min((-d2+sqrt(d2^2-4*d1*d3))/(2*d1),(-d2-sqrt(d2^2-4*d1*d3))/(2*d1)); %
        if CT == 3
            %%% Rubisco Limited
            JC = Vm*(Cc -GAM)/(Cc + Kc*(1+Oa/Ko)); %%% Gross Assimilation Rate Limited by Rubisco% [umolCO2/ s m^2 ]
            %%% Light Limited
            JL = (J/4)*(Cc -GAM)/(Cc + 2*GAM); %%% Gross Assimilation Rate Limited by Light % [umolCO2/ s m^2 ]
            %%% Capacity of the leaf to export or utilize the products of photosynthesis
            JE = 3*TPU; %0.5*Vm; %% Gross Assimilation /Rate Limited by Export % [umolCO2/ s m^2 ]
        elseif CT==4
            %%% Rubisco Limited
            JC = Vm; %%% Gross Assimilation Rate Limited by Rubisco% [umolCO2/ s m^2 ]
            %%% Light Limited
            JL = J/4; %%% Gross Assimilation Rate Limited by Light % [umolCO2/ s m^2 ]
            %%% PEP Carboxylase Limited
            JE =  20000*Vm*(Cc/Pre)*(CT==4); %% Gross Assimilation Rate Limited by Transport % [umolCO2/ s m^2 ]
        end
end
...
%%% First Polynomium
if CT==3
    b1= 0.9; b2= -(JC+JL); b3= JC*JL ;  %% b1 =0.98  Collatz 1991  Selleers1996b; Bonan et al 2011 ;  b1 = 0.83 Cox 1998
elseif CT==4
...
switch ANSTPU
    case 1
        %%% Second Polynomium
        if CT == 3
            c1 = 0.9; c2 = -(JP +JE); c3= JP*JE;  %%% c1 =0.95  Collatz 1991 Sellers1996b; Bonan et al 2011;   c1 =0.90 Cox1998
        elseif CT == 4
    ...
    case 2
        A=JP; %Rogers et al. (2020) suggest NOT TO USE TPU limitation!
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%fO = (1-Rgsw) ;%%%%%%
%fO(fO>1)=1;  fO(fO<0)=0;
fO=1; % we don't consider water stress at the moment MC
...
switch ANSGS
    case 1
        gsCO2 = go + (1+g1/sqrt(Ds))*An*Pre/Csl; %%%  [umolCO2 / s m^2] -- Stomatal Conductance
        NPQ=NaN;
        Fvp=NaN;
        Fmp=NaN;
    %%%%%% Medlyn et al. (2011)
    case 2
        [qL,NPQ,Fvp,Fmp] = PQredox(J,IPAR);
        gsCO2 = go + (1+g1/sqrt(Ds))*(1-qL)*Pre/Csl; %%%  [umolCO2 / s m^2] -- Stomatal Conductance
    %%%%%% Kromdijk et al., 2019
end
gsCO2(gsCO2<go)=go;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
rsCO2=1/gsCO2; %%% [ s m^2 / umolCO2 ] Stomatal resistance or Canopy
%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%
rsH20 = (rsCO2/1.64)*(10^6); %%% [ s m^2 / molH20 ] Stomatal resistance or canopy
An = (Csl - CcF)/(Pre*(rsCO2 + rmes + 1.37*rb + ra)); %%% Net Assimilation Rate  % [umolCO2/ s m^2 ]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
CcF = CcF/(Pre*10^-6); %% [umolCO2 /molAIR ]
rs = rsH20*(Tf*Pre)/(0.0224*(Ts+273.15)*Pre0); %% [s/m]  Stomatal resistance or Canopy
