...
Ta = 22; % Ambient temperature [°C]
Pa = 101325; % Atmospheric pressure [Pa]
RH = 66.3; % Relative humidity [%]
Ca = 504.6; % Ambient CO2 concentration [umol/mol]
...
TOTPFD=[270]; % imposed total PFD [umol m-2 s-1]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
S = [WUE./max(WUE) 1.64e-03.*gsCO2./max(gsobs)];
O = [WUEobs'./max(WUEobs) gsobs'./max(gsobs)];
